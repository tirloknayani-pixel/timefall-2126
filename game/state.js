...
function createGameState(playerIds) {
  const players = {};
  for (const [i, id] of playerIds.entries()) players[id] = { health: 100, action: null, submitted: false, x: 120 + (i % 3) * 90, y: 390 + Math.floor(i / 3) * 55, vx: 0, vy: 0 };
  return {
    round: 1,
...
function publicGameState(state) {
  return {
    ...
    players: Object.fromEntries(Object.entries(state.players).map(([id, p]) => [id, { health: p.health, submitted: p.submitted, x: p.x, y: p.y }])) ,
